...
| **AI Reasoning Layer** (SafeClaw) | Read ML outputs + market context, propose specific device commands with natural-language rationale                                            | Contextual reasoning. Proposes actions but cannot execute them. |
...
| UR-10 | The operator shall be able to adapt the system to different fleet compositions and site conditions. | Tether operates heterogeneous fleets (6+ ASIC models) across sites with varying ambient conditions and energy pricing.                                                 |
...
| ID        | Decision                                                                                                                    | Quality attribute                           | Rationale                                                                                                                                                                                                | Swappable                                            | Traces to       |
| --------- | --------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | --------------- |
| DR-POR-01 | ML layer and reasoning layer are decoupled via JSON file interface. ML output is `fleet_actions.json`, not a function call. | Testability, independent deployment         | Either layer can be tested, replaced, or run independently. The ML layer is deterministic and reproducible; the reasoning layer is contextual and non-deterministic. The file interface is the contract. | gRPC, message queue, shared database                 | SR-AR-01        |
...
| UR-09 | SR-CO-02 | DR-POR-02                       | `workflows/fleet_simulation.py`, `scripts/simulation_loop.py`      | (Validance E2E)                                                                        |
