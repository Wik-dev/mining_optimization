...
- ⚠️ **Low-temperature alert** — add 20°C floor to controller (hydro site in northern climate)
- ⚠️ **Map `set_voltage` to `setFrequency`** — MOS doesn't expose direct voltage control; note V/f coupling
- ⚠️ **MOS error codes** — map our anomaly types to MOS error code taxonomy in the report
- ⚠️ **Action approval system** — mention MOS's multi-voter pattern as a safety layer in the security section
- ⚠️ **Hashrate resolution** — note MOS provides 5s/5m/30m granularity vs our single value
- ⚠️ **Use W/TH/s convention** — MOS uses W/TH/s, we use J/TH (same unit, different label)
